...
<header itemtype="http://schema.org/WPHeader" itemscope="itemscope" id="masthead" class="site-header is-transparent <?php if ( 1 === get_theme_mod( 'whitedot_sticky_transparent_header', 0 ) ) { ?>transparent-fixed-head<?php } ?>">
	...
	<div class="transparent-main-header clear <?php if ( 1 === get_theme_mod( 'whitedot_sticky_transparent_header', 0 ) ) { ?>transparent-main-fixed-head<?php } ?>">
...
function whitedot_temp_header_logo(){
	...
	if ( has_custom_logo() ) {?>
		<div class="wd-site-logo <?php if ( get_theme_mod( 'whitedot_home_temp_logo' ) ) : ?>temp-logo<?php endif; ?>"> 
			<span>
				<?php the_custom_logo(); ?>
			</span>
		</div>
...
?>
	...
	<div class="site-branding" <?php if ( has_custom_logo() || get_theme_mod( 'whitedot_home_temp_logo' ) ) : ?>style="position: absolute; font-size: 1px; top: -300px;"<?php endif; ?>>
		<?php 
			if ( is_front_page() && is_home() ) : ?>
				<h1><a itemprop="url" class="site-name" href="<?php echo esc_url( home_url() ); ?>"><span itemprop="name"><?php bloginfo('name'); ?></span></a></h1>
		 <?php else :?>
